    ...
    <div class="text-white mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-3 gap-x-8">
            ...
            <div class="col-span-3 md:col-span-1">
                <div class="bg-white p-4 rounded-md text-gray-900 mt-4 flex">
                    <img class="w-20 border-2 border-gray-900" src="{{ asset('storage/' . $event->organization->image) }}"
                        alt="">
                    <div class="ml-8">
                ...
                <dl class="my-4 space-y-6">
                    ...
                    @if ($reminder)
                        <button disabled type="button"
                            ...
                            Pengingat sudah disetel pada tanggal
                            {{ \Carbon\Carbon::parse($reminder->remind_at)->translatedFormat('l, d F Y H:i') }}
                        </button>
                    @elseif($event->status != config('constants.EVENT.STATUS.2'))
                                        ...
                                        <div x-show="modal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-3 text-center sm:mt-5">
                                                    ...
                                                    <div class="mt-2">
                                                        <p class="text-sm text-gray-500">Reminder akan dikirim ke email
                                                            Anda.</p>
                                                    </div>
                                                </div>
                                                <form class="mt-2"
        ...
        <div class="grid md:grid-cols-2 gap-x-4">
                    ...
                    <ul>
                        ...
                        @foreach ($event->materials as $material)
                            <li class="text-pink-500 font-medium text-lg">
                                <a target="_blank"
                                    href="{{ asset('storage/' . $material->path) }}">{{ $i++ . '. ' . $material->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                @else
            ...
            <div class="bg-white my-4 p-4 rounded-md shadow-md text-gray-900">
                ...
                @if (request()->user()->student->didAttendEvent($event))
                    <h2 class="text-green-500">Anda sudah absen di acara ini.</h2>
                @elseif ($event->attendance_open)
            ...
            <div class="bg-white my-4 p-4 rounded-md shadow-md text-gray-900">
                ...
                @if (!request()->user()->student->didAttendEvent($event))
                    <h2 class="text-red-500">Anda belum mengisi presensi acara.</h2>
                @elseif (!$feedback_exists)
                    <form class="mt-4 flex flex-col gap-y-2"
                        ...
                        <div>
                            ...
                            <select id="rating" name="rating"
                                class="mt-1 block w-full rounded-md border-gray-300 py-2 pl-3 pr-10 text-base focus:border-pink-500 focus:outline-none focus:ring-pink-500 sm:text-sm" required>
                        ...
                        <div>
                            <label for="body" class="block text-sm font-medium text-gray-700">Masukan tentang
                                acara</label>
                            <div class="mt-1">
                                <textarea rows="4" name="body" id="body" required
