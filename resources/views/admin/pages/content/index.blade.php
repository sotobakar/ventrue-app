                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach ($contents as $content)
                                <tr>
                                    ...
                                    @if ($content->event->status == 'Belum dimulai')
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-red-500">
                                            {{ $content->event->status }}
                                        </td>
                                    @elseif($content->event->status == 'Sedang berlangsung')
                                    ...
                                    <td x-data="{ open: false }"
                                        ...
                                        <div x-show="open" class="relative z-10" aria-labelledby="modal-title"
                                            ...
                                            <div class="fixed inset-0 z-10 overflow-y-auto">
                                                <div
                                                    ...
                                                    <div
                                                        ...
                                                        <div>
                                                            <h3>Pilih Acara untuk ditampilkan di urutan {{ $content->id }}
                                                            </h3>
                                                            <form id="{{ 'update_content_' . $content->id }}" class="mt-2" action="{{ route('admin.content.update', ['content' => $content->id]) }}" method="POST">
                                                                @method('PUT')
                                                                @csrf
                                                                <div>
                                                                    <label for="event_id"
                                                                        class="block text-sm font-medium text-gray-700">ID Acara</label>
                                                                    <div class="mt-1">
                                                                    ...
                                                                    <p class="mt-2 text-sm text-gray-500"
                                                                        id="event-description">ID Acara bisa ditemukan pada url acara/{id acara}.
                                                                    </p>
                                                                </div>
                                                            </form>
                                                        </div>
                                                        <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                                            <button form="{{ 'update_content_' . $content->id }}" type="submit"
